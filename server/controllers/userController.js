...
export const getAcceptedStudents = async (req, res) => {
  try {
      const acceptedStudents = await User.find({ status: 'Accepted', role: 'Student' });

      res.status(200).json({ success: true, count: acceptedStudents.length, data: acceptedStudents });
  } catch (error) {
